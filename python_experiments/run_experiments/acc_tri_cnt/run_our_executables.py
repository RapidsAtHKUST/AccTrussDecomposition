...
def run_exp(env_tag=knl_tag, data_path_tag=exec_path_tag):
    ...
    # our_exec_name_lst = [name + '-non-sym-assign' for name in our_exec_name_lst]
    # our_exec_name_lst += filter(lambda exec_name: 'cuda' not in exec_name, my_config_dict[exec_lst_tag])
    # our_exec_name_lst = filter(lambda exec_name: 'hbw' not in exec_name, our_exec_name_lst)
    # our_exec_name_lst = filter(lambda exec_name: 'baseline' in exec_name, our_exec_name_lst)
    ...
    # data_set_lst = filter(lambda name: 'twitter' in name, data_set_lst)
    # print data_set_lst
    exp_res_root_name = 'exp_results'
    # folder_name = 'exp-03-23-stat' + '-' + hostname
    folder_name = 'exp-03-21-study-all-edge-counting'

    # parameters
    eps_lst = [0.2]
    mu_lst = [5]

    # reorder_method_lst = ['bfsr', 'dfs', 'hybrid', 'cache', 'rcm-cache', 'slashburn']
    # reorder_method_lst = ['gro']
    reorder_method_lst = ['gro', 'org', 'deg', 'kcore']

    # reorder_method_lst = ['deg']
                            ...
                            for reorder_method in reorder_method_lst:
                                statistics_dir = os.sep.join(
                                    map(str,
                                        ['.',
                                         ...
                                         data_set_name, reorder_method, eps, mu, t_num]))
                                ...
                                params_lst = map(str,
                                                 [algorithm_path,
                                                  ...
                                                  eps, mu, t_num, reorder_method, statistics_file_path,
                                                  '> /dev/null 2>&1'])
                                cmd = ' '.join(params_lst)
                                # print 'cmd: ', cmd
                                time_out = 3600
