...
def run_exp(env_tag=knl_tag):
    ...
    our_exec_name_lst = filter(lambda exec_name: 'multi-pass' in exec_name, my_config_dict[exec_lst_tag])
    # print 'our_exec_name_lst', our_exec_name_lst
    data_set_lst = my_config_dict[data_set_lst_tag]

    exp_res_root_name = 'exp_results'
    folder_name = 'overview-10-18-lb-gpu'
                    ...
                    for mu in mu_lst:
                        for t_num in thread_num_lst:
                            statistics_dir = os.sep.join(
                                map(str,
                                    ['.', exp_res_root_name, folder_name + '-' + str(num_of_gpus), data_set_name, eps,
                                     mu, t_num]))
                            ...
                            my_env["CUDA_VISIBLE_DEVICES"] = ','.join(map(str, range(0, 0 + num_of_gpus)))
    ...
    # normal case
    for num_gpus in list(reversed(range(1))):
        one_round(is_rev_deg=True, num_of_gpus=num_gpus)
        one_round(is_rev_deg=False, num_of_gpus=num_gpus)
