...
def run_exp(env_tag=knl_tag, with_c_group=True, data_path_tag=k_truss_exec_path_tag):
    ...
    folder_name = 'exp-2019-10-05-cuda-nvprof' + os.sep + hostname
    org_order_lst = ['org']
    our_reorder_dict = {
            ...
            for reorder_method in filtered_reorder_lst:
                for our_algorithm in our_exec_name_lst:
                    for t_num in thread_num_lst:
                        ...
                        statistics_file_path = statistics_dir + os.sep + our_algorithm + '-nvprof.log'
                        dstat_file_path = os.sep.join([statistics_dir, 'dstat', our_algorithm + '-nvprof-dstat.log'])
                        log_file_path = os.sep.join([statistics_dir, 'log', our_algorithm + '-nvprof-raw.log'])
                        ...
                        params_lst = map(str, ['cgexec -g memory:yche-exp' if with_c_group else '',
                                               'nvprof', algorithm_path, data_set_path + os.sep + data_set_name,
                                               reorder_method,
                                               statistics_file_path])
                        cmd = ' '.join(params_lst)
                        logger.info('exec-cmd: {}'.format(cmd))
                        time_out = 3600 * 5 if 'cuda' not in our_algorithm else 3600
