...
config_10_17_lb_gpu = ['overview-10-17-varying-num-pass-1', '10-17-varying-num-pass.json',
                       '10-17-varying-num-pass.md']
stat_config_lst = config_09_25_lb_gpu
my_config_lst = [config_10_17_lb_gpu]

algorithm_lst = ['scan-xp-cuda-experimental-hybrid-kernels-varying-multi-pass',
                 'scan-xp-cuda-experimental-bitmap1D-varying-multi-pass',
                 'scan-xp-cuda-experimental-bitmap-varying-multi-pass']
algorithm_name_dict = {
    'scan-xp-cuda-experimental-hybrid-kernels-varying-multi-pass': 'MPS',
    'scan-xp-cuda-experimental-bitmap1D-varying-multi-pass': 'BMP',
    'scan-xp-cuda-experimental-bitmap-varying-multi-pass': 'BMP-RF'
}
...
def generate_statistics(platform_tag):
    ...
    filtered_data_set_lst = filter(lambda name: 'small' not in name, config_dict[data_set_lst_tag])
    rev_deg_order_data_set_lst = map(lambda name: name + os.sep + rev_deg_order_tag,
                                     filtered_data_set_lst)
    num_of_gpus = str(1)
    num_of_pass_lst = map(str, range(1, 16))
    ...
    def fetch_data_to_json():
        ...
        for algorithm in algorithm_lst:
            ...
            for data_set in rev_deg_order_data_set_lst:
                my_dict[data_set] = {}
