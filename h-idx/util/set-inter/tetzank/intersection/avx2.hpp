...
#ifdef USE_ALIGNED
#define MM256_LOAD_SI_256 _mm256_load_si256
#else
#define MM256_LOAD_SI_256 _mm256_loadu_si256
#endif

#include "util/set-inter/tetzank/shuffle_dictionary.hpp"
#include "naive.hpp"

inline size_t intersect_vector_avx2(const uint32_t *list1, size_t size1, const uint32_t *list2, size_t size2, uint32_t *result){
	size_t count=0, i_a=0, i_b=0;
#ifdef __AVX2__
	size_t st_a = (size1 / 8) * 8;
	size_t st_b = (size2 / 8) * 8;
	while(i_a < st_a && i_b < st_b){
		__m256i v_a = MM256_LOAD_SI_256((__m256i*)&list1[i_a]);
		__m256i v_b = MM256_LOAD_SI_256((__m256i*)&list2[i_b]);
...
// 		int32_t b_max = _mm256_extract_epi32(v_b, 7);
		...
		__m256i idx = MM256_LOAD_SI_256((const __m256i*)&shuffle_mask_avx[mask*8]);
...
inline size_t intersect_vector_avx2_count(const uint32_t *list1, size_t size1, const uint32_t *list2, size_t size2){
	size_t count=0, i_a=0, i_b=0;
#ifdef __AVX2__
	size_t st_a = (size1 / 8) * 8;
	size_t st_b = (size2 / 8) * 8;
...
/*
 * Replace vmovdqu with vmovdqu
 */
inline size_t intersect_vector_avx2_asm(const uint32_t *list1, size_t size1, const uint32_t *list2, size_t size2, uint32_t *result){
	size_t count=0, i_a=0, i_b=0;
#ifdef __AVX2__
	size_t st_a = (size1 / 8) * 8;
	size_t st_b = (size2 / 8) * 8;
	...
	"1: "
		...
		"vmovdqu ymm1, [%q[list1] + %q[i_a]*4];" // elements are 4 byte
		"vmovdqu ymm2, [%q[list2] + %q[i_b]*4];"
		...
		"movsxd r8, r9d;"
		"shl r8, 5;"
		"vmovdqu ymm0, [%q[shuffle_mask] + r8];"
...
inline size_t intersect_vector_avx2_asm_count(const uint32_t *list1, size_t size1, const uint32_t *list2, size_t size2){
	size_t count=0, i_a=0, i_b=0;
#ifdef __AVX2__
	size_t st_a = (size1 / 8) * 8;
	size_t st_b = (size2 / 8) * 8;
